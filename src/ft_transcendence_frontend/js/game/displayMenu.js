...
export class MenuDisplay {
                        ...
                        <div class="modal-body" style="background-color: #b9a84cc0;">
                            <form id="edit-profile-form">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="date" id="edit-birth-date" class="form-control" value="${
                                      this.userProfile.birth_date || ""
                                    }">
                                </div>
                                <div class="mb-3">
                                    <label for="edit-tournament-name" class="form-label">Tournament Name:</label>
                                    <input type="text" id="edit-tournament-name" class="form-control" value="${
                                      this.userProfile.tournament_name || ""
                                    }">
                                </div>
                            </form>
                        </div>
    ...
    document
      ...
      .addEventListener("click", async () => {
        ...
        try {
          ...
          if (this.elements.birthDate)
            this.elements.birthDate.textContent = birthDate;
          if (this.elements.tournamentName)
            this.elements.tournamentName.textContent = tournamentName;
  ...
  updateProfileDisplay(profileData) {
    ...
    if (this.elements.birthDate)
      this.elements.birthDate.textContent = profileData.birth_date || "";
    if (this.elements.tournamentName)
      this.elements.tournamentName.textContent =
        profileData.tournament_name || "";
                            ...
                            <div class="card-body profile-card">
                                <div class="row">
                                    ...
                                    <div class="col-md-8 col-sm-12">
                                        <h4 class="profile-username mb-3">Welcome ${
                                          this.userProfile.username
                                        }!</h4>
                                        <div class="profile-details mb-3">
                                            <p class="mb-2"><strong>Email:</strong> <span class="profile-email">${
                                              this.userProfile.email
                                            }</span></p>
                                            <p class="mb-2"><strong>Bio:</strong> <span class="profile-bio">${
                                              this.userProfile.bio || ""
                                            }</span></p>
                                            <p class="mb-2"><strong>Birthday:</strong> <span class="profile-birth-date">${
                                              this.userProfile.birth_date || ""
                                            }</span></p>
                                            <p class="mb-2"><strong>Tournament Name:</strong> <span class="profile-tournament-name">${
                                              this.userProfile
                                                .tournament_name || ""
                                            }</span></p>
                                        </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body profile-card">
                        <div class="mb-3">
                            ...
                            <input type="number" id="ball-speed" class="form-control" min="1" max="10" value="${
                              currentSettings.ball_speed
                            }">
                        </div>
                        <div class="mb-3">
                            <label for="paddle-speed" class="form-label">Paddle Speed (1-10):</label>
                            <input type="number" id="paddle-speed" class="form-control" min="1" max="10" value="${
                              currentSettings.paddle_speed
                            }">
                        </div>
                        <div class="mb-3">
                            <label for="winning-score" class="form-label">Winning Score:</label>
                            <input type="number" id="winning-score" class="form-control" min="1" max="20" value="${
                              currentSettings.winning_score
                            }">
                        </div>
                        <div class="mb-3">
                            ...
                            <select id="paddle-size" class="form-select">
                                <option value="small" ${
                                  currentSettings.paddle_size === "small"
                                    ? "selected"
                                    : ""
                                }>Small</option>
                                <option value="middle" ${
                                  currentSettings.paddle_size === "middle"
                                    ? "selected"
                                    : ""
                                }>Middle</option>
                                <option value="big" ${
                                  currentSettings.paddle_size === "big"
                                    ? "selected"
                                    : ""
                                }>Big</option>
                            </select>
  ...
  displayPlayerNamesInput(numPlayers, isTournament) {
    ...
    document.getElementById("player-names-form").onsubmit = (e) => {
      ...
      const playerNames = Array.from(
        { length: numPlayers },
        (_, i) => document.getElementById(`player-${i + 1}`).value
      );
            ...
            <div class="container py-5">
                <div class="card text-center">
                    ...
                    <div class="card-body profile-card py-5">
                        <h3 class="mb-4" id="search-status-message">${
    ...
    try {
      ...
      if (friendsList) {
        ...
        friends.forEach((friend) => {
          ...
          listItem.className =
            "list-group-item d-flex flex-column align-items-center text-center";
  ...
  addChatMessage(sender, message, isOwnMessage = false) {
    ...
    messageElement.innerHTML = `
            ${
              sender !== "System" ? `<strong>${sender}:</strong> ` : ""
            }${message}
        `;
  ...
  async blockUser(username) {
    if (
      confirm(
        `Are you sure you want to block ${username}? You will no longer be able to exchange messages.`
      )
    ) {
      try {
  ...
  async isUserBlocked(username) {
    try {
      ...
      console.error(
        "Fehler beim Prüfen, ob der Benutzer blockiert ist:",
        error
      );
      return false;
    }
    ...
    if (
      ...
      try {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          alert(
            `Error unblocking user: ${errorData.detail || "Unknown error"}`
          );
        }
      } catch (error) {
...
OnlineUsersHandler.updateOnlineUsersList = function (
  onlineUsers,
  friendsHandler
) {
  ...
  if (onlineUsers.length === 0) {
    if (
      ...
      (usersList.children.length === 1 &&
        !usersList.children[0].classList.contains("no-players"))
    ) {
      usersList.innerHTML =
        '<li class="list-group-item text-center no-players">No players online</li>';
    }
    return;
  }
  ...
  friendsHandler
    ...
    .then((friends) => {
      ...
      const currentUsername = window.menuDisplay
        ? window.menuDisplay.userProfile.username
        : "";
...
FriendsHandler.prototype.getFriends = async function () {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Fehler beim Abrufen der Freundesliste: ${response.status} ${response.statusText}`
      );
    }
    ...
    console.error(
      "FriendsHandler: Fehler beim Abrufen der Freundesliste:",
      error
    );
    return [];
  }
